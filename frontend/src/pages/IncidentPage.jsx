...
function IncidentListView() {
  ...
  const fetchIncidents = useCallback(async (s, st, sv, cat, pg) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(() => {
      fetchIncidents(search, filterStatus, filterSeverity, filterCategory, page);
    }, 300);
    return () => clearTimeout(searchTimeout.current);
  }, [search, filterStatus, filterSeverity, filterCategory, page, fetchIncidents]);

  function clearFilters() {
    setSearch(''); setFilterStatus(''); setFilterSeverity(''); setFilterCategory(''); setPage(1);
  }
      ...
      <main className="ml-60 flex-1 p-6">
        ...
        <div className="card p-3 mb-4 flex flex-wrap gap-2 items-center">
          <input
            ...
            placeholder="Search by ref, title, summary..."
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['Ref', 'Received', 'Category', 'Severity', 'Status', 'Department', 'SLA', ''].map(h => (
                ...
                {incidents.map((inc) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <SLATimer sla_deadline={inc.sla_deadline} is_overdue={inc.is_overdue} sla_hours={inc.sla_hours} />
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-dhl-red text-xs font-medium hover:underline">View →</span>
                    </td>
                  </tr>
                ))}
              </tbody>
...
function IncidentDetailView({ id }) {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen">
        <Navbar />
        <main className="ml-60 flex-1 flex items-center justify-center text-gray-400">Loading...</main>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="flex min-h-screen">
        <Navbar />
        <main className="ml-60 flex-1 flex items-center justify-center text-gray-400">Incident not found</main>
      </div>
    );
  }
  ...
  function exportAuditCSV() {
    ...
    a.href = url;
    a.download = `audit_trail_${incident.incident_ref}.csv`;
    a.click();
          ...
          <div className="px-6 py-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="ml-auto">
                <SLATimer sla_deadline={incident.sla_deadline} is_overdue={incident.is_overdue} sla_hours={incident.sla_hours} />
              </div>
            </div>
          </div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full transition-all ${
                          incident.llm_confidence >= 0.8 ? 'bg-green-500' :
                          incident.llm_confidence >= 0.6 ? 'bg-amber-500' : 'bg-red-500'
                        }`}
              ...
              <div className="card p-5">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm italic text-gray-500">{incident.root_cause_suggestion || '—'}</p>
                  </div>
                </div>
              </div>
                    ...
                    <>
                      ...
                      <div className="mt-4">
                        <button
                          ...
                          className="bg-dhl-red text-white text-sm font-semibold px-4 py-1.5 rounded-md hover:bg-dhl-red-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                        >
          ...
          {activeTab === 'sources' && (
            <div className="space-y-4">
              <div className="mb-2">
                ...
                <p className="text-gray-500 text-sm mt-0.5">{raw_inputs.length} source(s) identified and processed by UiPath</p>
              </div>
              {raw_inputs.length === 0 ? (
                <div className="card p-8 text-center text-gray-400 text-sm">No source inputs linked</div>
              ) : raw_inputs.map((ri) => (
                <SourceCard key={ri.id} input={ri} />
              ))}
            </div>
          )}
          ...
          {activeTab === 'departments' && (
            <div className="space-y-5">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {primaryTask.task_status === 'Not Started' && (
                      <button
                        onClick={() => handleTaskUpdate(primaryTask.id, 'In Progress')}
                    ...
                    {primaryTask.task_status === 'In Progress' && (
                      <button
                        onClick={() => handleTaskUpdate(primaryTask.id, 'Completed')}
                      ...
                      <div key={task.id} className="card p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            <p className="font-semibold text-gray-800 text-sm">{task.department}</p>
                            <p className="text-gray-600 text-sm mt-1">{task.task_description}</p>
                            <p className="text-gray-400 text-xs mt-1.5">Updated {timeAgo(task.updated_at)}</p>
                          </div>
              ...
              <div className={`card p-5 border-2 ${allComplete ? 'border-green-200 bg-green-50' : 'border-gray-100'}`}>
                ...
                {allComplete ? (
                  <button
                    onClick={() => handleStatusUpdate_direct('Resolved')}
                  ...
                  <div>
                    <button
                      disabled
                      title="All department tasks must be completed first"
  ...
  async function handleStatusUpdate_direct(newStatus) {
    try {
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      await api.patch(`/incidents/${id}/status`, { status: newStatus, actor: user.name || 'Agent' });
      await fetchDetail();
    } catch (err) {
      alert(err.response?.data?.error || 'Failed to update status');
    }
        ...
        <div className="bg-gray-50 rounded p-3 text-xs text-gray-700 leading-relaxed font-mono whitespace-pre-wrap">
          ...
          {hasMore && (
            <button
              ...
              className="block mt-2 text-dhl-red hover:underline font-sans font-medium"
            >
      ...
      {input.content_type === 'image' && input.ocr_confidence != null && (
        <div className="mt-3">
          ...
          <div className="w-full bg-gray-200 rounded-full h-1.5">
            <div
              className={`h-1.5 rounded-full ${input.ocr_confidence >= 0.8 ? 'bg-green-500' : input.ocr_confidence >= 0.5 ? 'bg-amber-500' : 'bg-red-500'}`}
